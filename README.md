...
**1.0.88** is the current implementation snapshot. It carries the tested End Resonance portal flow, Entity Scanner/Codex workflow and runtime-QA corrections toward the 1.1.0 release candidate. Opening a source workspace is navigation only; `Use <mode> mode` is the sole action that changes the active source. Table supports Image, Entity and Banner, whereas Wall remains Image-only. Its floating core now matches the Wall core's scale with no glass chamber; the Light Projector has separate handcrafted Off, Focus/Flood and Ambient models with geometry-derived collision; the Entity workspace also supports a bodyless, poseable mannequin; fish retain their native aquatic orientation and stable swimming animation; fitted previews reserve margin for large entities while using one bounded, symmetric mouse orbit for every entity renderer; and every projector control is a live server-synchronized edit. Network protocol is **44** and `ProjectionSettings` format remains **4**.
