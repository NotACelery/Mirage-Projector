...
Since 1.0.13, **Mirage Equipment** adds a dedicated Shoulder Strap + Shoulder Slot without consuming armor or offhand space. The Strap owns its mounted device, item compartment and upgrades, so a packed Strap behaves as a portable mini-container when removed, stored or swapped. Its shared Survival/Creative inventory panel exposes eight base item slots in two rows and twelve with **Shoulder Strap Slot Expansion**; accepted contents are rechargeable media, Flashlights, Hand Projectors, Scan Codices and empty or filled Entity Scan Cards. Two base upgrade sockets expand to three. Auto Battery Swap remains shoulder-only. Since 1.0.15 the directional Charging Station provides four queued inputs, one active charger, four outputs and front-face logistics. Banner-profile Hand Projectors can use the pole-free overhead War Banner presentation with body-attached Directional or per-viewer billboard facing.
...
**1.0.117** is the current implementation snapshot. It carries the tested End Resonance and Entity Scanner/Codex workflows plus the stabilized portable-projector and Shoulder Strap pass. Survival and Creative use the same shoulder overlay, cursor synchronization and slot rules. Packed Straps retain their mounted device, stored items and upgrades; external-slot clicks no longer use vanilla's outside-inventory drop path. War Banners share one body-relative anchor, hide in first person, and expose continuous 0–100% size and -32–+32 px height controls. Network protocol is **44** and `ProjectionSettings` format remains **4**.
