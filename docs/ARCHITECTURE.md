...
## 1.0.19 packed-light visual bridge / 1.0.20 Sodium hotfix / 1.0.21 invalidation follow-up

`DYNAMIC_VISUAL` remains client-owned and does not mutate authoritative vanilla block-light propagation. After the virtual field is solved and affected sections are dirtied, `LevelRendererMirageLightMixin` merges the Mirage block channel into the packed light coordinate returned by both `LevelRenderer.getLightColor(...)` overloads. The vanilla sky channel is preserved. This closes the architectural gap exposed by runtime QA where external light-level queries saw Mirage values but baked terrain vertices remained visually dark. In 1.0.20 the bridge reads the Mirage field from the active `ClientLevel` instead of the generic render view, avoiding Sodium `LevelSlice#getLightEngine()` during chunk meshing. In 1.0.21 invalidation is render-dependency aware: static updates refresh a full one-section halo, while moving emitters compare pre/post aggregate bytes and only dirty neighboring sections when changed boundary voxels can be sampled by their meshes.
