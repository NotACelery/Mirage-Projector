...
dev.69 defines the downstream reflected-light branch specifically for an energized Mature Crying Obsidian Cluster. Small/Medium/Large buds remain zero-block-light optical stages. Mature powered light decays at approximately half vanilla speed through Mirage-owned nodes. Reflected material identity is no longer inferred from generic relay width: Glass broadens coverage through bounded diagonal Diffusion, Quartz/Radiance is the dominant range amplifier, Amethyst drives residual Resonance, Diamond adds smaller focused axial reach while counteracting Diffusion, and Netherite preserves Inversion. Every real source/node remains capped to vanilla level 15. dev.66's reusable `LightProfile` foundation remains the architecture for future profiles.
