# Cores and Projector Upgrades — authoritative design contract (dev.43)
...
>
> **Implemented systems are still governed by current source + `CURRENT-IMPLEMENTATION-AUDIT-dev43.md`.** Sections explicitly marked future remain design contracts; dev.43 material/Core-Chamber items listed as implemented are present in source.
>
> Crying-Obsidian farming/crystal/Beacon-absorption details live in `CRYING-OBSIDIAN-ECOSYSTEM-dev43.md`; dev.43 source now implements that crystal wave, while Improved-Core coupling remains future.
...
Full shard acquisition/growth contract: `CRYING-OBSIDIAN-ECOSYSTEM-dev43.md`.

---

# 3. Current broad Glass model layers are deprecated

dev.43 replaces the dev.40/dev.41 thin/broad Glass/Glass-Pane surfaces with composite solid structure + translucent Crying-Obsidian emitter geometry + a universal small Glass Core Chamber. The older ghost-sheet design is retired.
...
Detailed crystal behavior lives in `CRYING-OBSIDIAN-ECOSYSTEM-dev43.md`.
...
This document is one part of the broader roadmap in `CURRENT-STATE-ROADMAP-dev43.md`.
...
See `CRYING-OBSIDIAN-ECOSYSTEM-dev43.md` for the authoritative crystal-side rules.
