# Crying Obsidian Ecosystem — authoritative design contract (dev.43)

> **Status:** authoritative dev.43 contract. Crying Obsidian Shards, renewable Small/Medium/Large/Mature crystal growth, exact stage harvesting, structure loot, Beacon excitation/transmission and residual-beam rendering are implemented in dev.43 source candidate. Values marked provisional still require Windows build + in-game balance/render QA. Improved-Core coupling and extended-light >15 integration remain future.
>
...
**dev.43 implementation:** data-driven NeoForge Global Loot Modifiers are present for Ruined Portal, Abandoned Mineshaft and village armorer/toolsmith/weaponsmith chests.
...
**dev.43 implementation:** present in source. Vanilla Crying Obsidian is made random-tick eligible by a narrow Mixin hook; nucleation requires a Lava source above and grows only into the block below.
...
**dev.43 implementation:** block loot tables use Silk-Touch alternatives and exact 1/2/3/4 shard counts with no Fortune bonus.
...
**dev.43 implementation:** each age has an original unenergized and energized 16×16 cutout texture/model. Art remains subject to visual QA; the contract below governs future polish.
...
**dev.43 implementation:** client Beacon rendering is replaced only for active columns containing Mirage crystals. Vanilla Beacon gameplay remains active; stained-glass color sections are retained while Mirage applies age-dependent visual transmission.
...
**dev.43 implementation:** crystals poll Beacon excitation every 20 ticks and toggle an `energized` blockstate. Vanilla fallback light is 14/15/15/15 by age. Extended >15 output is not bundled yet.
...
**dev.43 implementation:** deterministic client rendering with solid-block ray clipping is present. Current active phases are 20 ticks extend + 20 ticks full hold + 20 ticks retract/fade; stage cycles are currently 180/160/140/120 ticks Small->Mature.
...
The dev.43 crystal/Beacon wave is now implemented as a source candidate. Remaining ecosystem work moves to Spike, projector upgrades and Improved Cores after QA.
...
- Obsidian Spike remains 3 shards + 2 String + 1 Stick, nine tips, 2.0 damage per successful hurt event;
...
# Improved-Core -> Beacon -> Crystal coupling (future, frozen in dev.43)

This interaction is **defined but not implemented in dev.43**. Powered crystals exist now; Improved-Core relay modifiers do not. When the relay wave arrives, its final effective beam must be resolved before crystal response is calculated.
