...
The suite covers historical regression contracts that remain relevant to 1.0.0 plus current release integrity checks.
...
`gradle.properties` is the single source of the project version used by the build metadata. Stable 1.0.0 uses:

```text
mod_version=1.0.0
```

Future development should branch from this baseline and move to the 1.1.0 development line rather than mutating the stable release snapshot.
