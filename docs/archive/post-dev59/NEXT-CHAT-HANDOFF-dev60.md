...
Windows QA revealed that extracting the project over an older folder can leave renamed Java sources behind. `build.bat` now deletes the four known obsolete pre-dev.59 source paths before compiling. The clean snapshot itself contains none of those stale classes.
