...
- dev.39 was executed in-game by the user; dev.40 is a source candidate built on that live baseline.
- POWER / CAPACITY no longer owns a section-wide hover tooltip. The detailed formula is opt-in through a small `?` hotspot.
- Core slot has a reserved left column; text starts to its right.
- A visible capacity/load progress bar is restored beneath the load metric.
- Projection-only Piglin/Hoglin clones are forced immune to zombification after frozen NBT load so vanilla conversion shaking cannot depend on the projector dimension. This affects both preview and world projection through the shared entity factory and never rewrites the scan.
- Network protocol remains **18**; no payload/NBT schema changed in dev.40.
- Improved Core recipes/items/variants are deliberately **not** part of dev.40. Next design pass must define each material and variant by role, effect, amplification, recipe materials and why it is useful before implementing assets/recipes.

Authority additions: `docs/ENTITY-DIMENSION-NORMALIZATION-dev40.md` and `docs/DEV40-UI-ENTITY-QA.md`.
