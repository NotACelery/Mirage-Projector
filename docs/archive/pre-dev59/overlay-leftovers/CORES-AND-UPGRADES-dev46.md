# Cores and Projector Upgrades — authoritative design contract (dev.46)

> **Status:** authoritative mixed contract for dev.46. Core Chamber/material language, complete chassis-upgrade progression and the five Improved Core block/item + projector-amplification implementations are source candidates. Improved-Core Beacon relay remains planned.
>
> **Implemented systems are still governed by current source + `CURRENT-IMPLEMENTATION-AUDIT-dev45.md`.** Sections explicitly marked future remain design contracts; dev.45 material/Core-Chamber items listed as implemented are present in source.
>
> Crying-Obsidian farming/crystal/Beacon-absorption details live in `CRYING-OBSIDIAN-ECOSYSTEM-dev45.md`; dev.45 source now implements that crystal wave, while Improved-Core coupling remains future.
...
Full shard acquisition/growth contract: `CRYING-OBSIDIAN-ECOSYSTEM-dev45.md`.

---

# 3. Current broad Glass model layers are deprecated

dev.45 replaces the dev.40/dev.41 thin/broad Glass/Glass-Pane surfaces with composite solid structure + translucent Crying-Obsidian emitter geometry + a universal small Glass Core Chamber. The older ghost-sheet design is retired.
...
Implemented dev.45 initial symmetric pair (material balance may still change after QA):
...
These patterns are **implemented in dev.45**. Their equal-cost geometry is now the functional baseline; later balance may adjust material counts only through an explicit design revision.
...
Implemented dev.45 initial recipe:
...
The Field pattern is implemented in dev.45 as the initial expensive specialization recipe. Exact cost remains balance-provisional after renewable-Crying-Obsidian and in-game model QA.
...
dev.45 implements a custom recipe serializer/assembly path; future upgrade work must reuse the canonical transfer architecture below.

---

# 11.4 dev.45 canonical transfer implementation

dev.45 implements the hard contract above with one canonical path:

```text
placed MirageProjectorBlockEntity
        ↓ BlockEntity.saveToItem
projector ItemStack + minecraft:block_entity_data
        ↓ crafting source
ItemStack.transmuteCopy(target chassis)
        ↓
load complete source payload into temporary target-chassis BlockEntity
        ↓ target load/migration/sanitization rules
BlockEntity.saveCustomOnly
        ↓
normalized minecraft:block_entity_data on result ItemStack
```

Consequences:

- ordinary Survival mining now returns **one projector item containing its complete persistent state** rather than a clean chassis plus separately ejected Core/card/staging inventory;
- middle-click clone also preserves the loaded projector state when a real Level is available;
- custom item-component patches survive chassis transmutation through vanilla `ItemStack.transmuteCopy`;
- source `BLOCK_ENTITY_DATA` is copied wholesale, so future fields written by the BlockEntity survive without adding them to recipe code;
- target compatibility normalization uses the target BlockEntity's own `loadAdditional` logic rather than a recipe-specific field list;
- a clean/unplaced Compact has no serialized state yet, so dev.45 materializes Compact defaults before upgrading. This preserves its implicit Glass Core;
- clean legacy Compact items are necessarily treated the same as newly crafted Compact items because older builds did not encode enough information to distinguish an intentionally empty Core socket from an unplaced default item;
- non-player destruction keeps the historical safety fallback: physical Core/card/staging contents are ejected rather than silently deleted, but virtual projector state is not promised for explosion/piston destruction in dev.45.

## 11.5 Implemented recipes

Base Mirage Projector from scratch:
...
- 1 Glass Block;
- 3 Obsidian;
- Compact still begins with its standard Glass Core when first placed.

Upgrade recipes are exact 3x3 custom recipes:

| Path | Pattern/cost status |
|---|---|
| Mirage -> Display | frozen Q/S/A pattern implemented |
| Display -> Wide | 6 shards + 2 Glass, horizontal pattern implemented |
| Display -> Tall | 6 shards + 2 Glass, vertical pattern implemented |
| Display -> Prism | 4 shards + 4 Glass implemented |
| Display -> Field | 4 Crying Obsidian + 4 shards implemented; balance provisional |

No direct Compact -> Wide/Tall/Prism/Field recipe exists. Display is the mandatory gateway.
...
Exactly five Improved Cores are implemented as dev.46 source candidates, one per standard material:
...
Implemented dev.46 QA candidate pattern:
...
This pattern is **implemented for live balance/art QA**. It remains balance-provisional until:
...
Detailed crystal behavior lives in `CRYING-OBSIDIAN-ECOSYSTEM-dev45.md`.
...
This document is one part of the broader roadmap in `CURRENT-STATE-ROADMAP-dev44.md`.
...
See `CRYING-OBSIDIAN-ECOSYSTEM-dev45.md` for the authoritative crystal-side rules.
