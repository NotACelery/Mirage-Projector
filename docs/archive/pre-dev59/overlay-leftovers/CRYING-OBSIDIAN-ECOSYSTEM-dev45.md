> **dev.45 note:** crystal growth/Beacon/shard behavior is unchanged from dev.44; this copy is promoted only so the current authority set is self-contained.

# Crying Obsidian Ecosystem — authoritative design contract (dev.45)

> **Status:** authoritative dev.45 contract. Crying Obsidian Shards, renewable Small/Medium/Large/Mature crystal growth, exact stage harvesting, structure loot, Beacon excitation/transmission and residual-beam rendering are implemented in dev.45 source candidate. Values marked provisional still require Windows build + in-game balance/render QA. Improved-Core coupling and extended-light >15 integration remain future.
>
...
**dev.45 implementation:** data-driven NeoForge Global Loot Modifiers are present for Ruined Portal, Abandoned Mineshaft and village armorer/toolsmith/weaponsmith chests.
...
**dev.45 implementation:** present in source. Vanilla Crying Obsidian is made random-tick eligible by a narrow Mixin hook; nucleation requires a Lava source above and grows only into the block below.
...
**dev.45 implementation:** block loot tables use Silk-Touch alternatives and exact 1/2/3/4 shard counts with no Fortune bonus.
...
**dev.45 implementation:** each age has an original unenergized and energized 16×16 cutout texture/model. Art remains subject to visual QA; the contract below governs future polish.
...
**dev.45 implementation:** client Beacon rendering is replaced only for active columns containing Mirage crystals. Vanilla Beacon gameplay remains active; stained-glass color sections are retained while Mirage applies age-dependent visual transmission.
...
**dev.45 implementation:** crystals poll Beacon excitation every 20 ticks and toggle an `energized` blockstate. Vanilla fallback light is 14/15/15/15 by age. Extended >15 output is not bundled yet.
...
**dev.45 implementation:** deterministic client rendering with solid-block ray clipping is present. Current active phases are 20 ticks extend + 20 ticks full hold + 20 ticks retract/fade; stage cycles are currently 180/160/140/120 ticks Small->Mature.
...
The dev.43 crystal/Beacon wave and dev.45 Obsidian Spike are implemented as source candidates. Remaining ecosystem work moves to projector upgrades and Improved Cores after QA.
...
# Improved-Core -> Beacon -> Crystal coupling (future, frozen in dev.45)

This interaction is **defined but not implemented in dev.45**. Powered crystals exist now; Improved-Core relay modifiers do not. When the relay wave arrives, its final effective beam must be resolved before crystal response is calculated.
...
# 11. Obsidian Spike — implemented dev.45
