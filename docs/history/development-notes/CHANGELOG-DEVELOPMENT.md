## 0.1.0-dev.86a — dev.86 compile hotfix

- Fixes the Windows `compileJava` failure in `CryingObsidianCraftingEmiRecipe`.
- Corrects the `ItemLike` import from `net.minecraft.world.item.ItemLike` to the NeoForge/Minecraft 1.21.1 location `net.minecraft.world.level.ItemLike`.
- No recipe-viewer UX, recipe data, gameplay, renderer, lighting or protocol behavior changes from dev.86.

## 0.1.0-dev.86 — EMI / JEI crafting-tab and block-drop ordering polish

- Fixed the missing EMI Crafting tabs for Mirage Display, Wide, Tall, Prism and Field projector upgrades. The previous integration removed the original custom-recipe IDs and then reused those same IDs for the replacement EMI recipes, so EMI's removal predicate also filtered the replacements. dev.86 uses separate synthetic EMI IDs.
- Added explicit EMI Crafting entries for Crying Obsidian made from 8 Crying Obsidian Shards plus either a Fire Charge or Magma Cream.
- Rescaled World Interaction arrows to 12×9 so growth-stage sprites no longer overlap them.
- Added deterministic Small → Medium → Large → Cluster ordering for Crying Obsidian crystal Block Drops by replaying the viewer-generated rows with age-prefixed synthetic IDs.
- JEI projector crafting support remains explicit through `ProjectorUpgradeCraftingExtension`; Crying Obsidian recipes remain normal shaped recipes discovered by JEI automatically.
- No gameplay/protocol/render/light changes.

## 0.1.0-dev.83 — EMI / JEI recipe-viewer integration

- Adds optional EMI and JEI development dependencies for NeoForge 1.21.1.
- Registers the custom Mirage projector chassis-upgrade recipes explicitly in EMI so Display/Wide/Tall/Prism/Field progression is visible and stable in the crafting viewer.
- Registers a dedicated EMI world-interaction entry for renewable Crying Obsidian growth, linked from Crying Obsidian, all bud stages, the cluster and the shard. The page is icon-only on purpose: lava above Crying Obsidian shows the four growth stages with tooltips instead of body text.
- Adds JEI crafting-category support for the custom projector-upgrade recipes and ingredient info pages for the projector upgrade line plus the renewable Crying Obsidian mechanic.
- Keeps runtime gameplay, rendering, lighting, GIF, Core and protocol behavior unchanged from the accepted dev.82 baseline.
