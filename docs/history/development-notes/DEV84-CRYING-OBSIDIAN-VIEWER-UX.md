# dev.84 — Crying Obsidian viewer UX cleanup
...
## dev.84 UX refinement

The original dev.83 EMI page explained Crying Obsidian growth as four separate towers, which made the mechanic look like four different setups. dev.84 replaces that with one practical setup: lava above Crying Obsidian, followed by a single growth row underneath (small → medium → large → cluster).
