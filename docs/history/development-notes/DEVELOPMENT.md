...
Current source line: **0.1.0-dev.86a**.
...
dev.61 removed the Create Backtank crash, dev.62 restored late Entity transform/depth, and dev.64 switched the synthetic Create chest to a single Ghost surface. dev.70–73 exposed the limits of physical auxiliary emitters; dev.74 validated the causal fixed-point Mirage solver; dev.75 established virtual source authority/occlusion/detour; dev.76 moves STATIC_WORLD to server-resolved section synchronization; dev.76f–h add revisioned chunk snapshots, client recovery/manifest checks and the section-boundary invalidation fix. The current protocol is 27. dev.82 bumps it because ProjectionSettings and SetProjectionSource now serialize stable source IDs plus versioned quaternion-ready transform data. dev.86 closes EMI/JEI recipe discoverability and ordering without changing protocol or gameplay. Repeated relog/chunk-load QA on dev.76h was accepted with no visible FPS regression; remaining release QA is tracked in `WAITLIST-1.0.0.md`.
...
Important solver rule retained from dev.75d: open traversal cost and obstacle-detour extra cost are independent parameters. Do not hard-code Cluster-specific wall rules in the solver. Since dev.76 the client no longer needs the profile over the wire for STATIC_WORLD; current protocol 27 transports revisioned server-resolved chunk snapshots plus lightweight revision manifests/requests instead.
