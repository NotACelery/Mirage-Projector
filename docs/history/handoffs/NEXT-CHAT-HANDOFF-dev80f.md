# NEXT CHAT HANDOFF — 0.1.0-dev.80f

Use **`0.1.0-dev.80f`** as the latest recoverable source snapshot. Network protocol remains **26**.
...
## dev.80f selective Mirage Projector first-person lift

- Follow-up to dev.80e after QA confirmed Mirage Display, Mirage Field Projector, Wide Mirage Projector, Tall Mirage Projector and Mirage Prism were finally acceptable in first-person.
- The only remaining first-person visibility outlier was the compact Mirage Projector.
- Therefore only `models/item/mirage_projector.json` changed in this pass.
- Compact Mirage Projector first-person translation Y is now `1.85`; the other five projector items remain at `1.05`.
- All slab-like scales from dev.80d and the broad first-person lift logic from dev.80e remain otherwise unchanged.
- Goal: finish the held-item pass with the smallest possible targeted adjustment.
- No block geometry, VoxelShape, registry, recipe, menu, power or protocol changes were introduced.

## Verification

Run `python tools/verify_current_line.py`, then Windows `build.bat`. The current verifier checks canonical promotion, the selective compact-projector first-person lift, creative ordering, Compact/Tall final QA fixes, resource coverage and the Mirage Light Engine regression gates.
