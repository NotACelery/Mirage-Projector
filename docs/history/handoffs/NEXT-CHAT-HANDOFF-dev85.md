# NEXT CHAT HANDOFF — 0.1.0-dev.85

Use **`0.1.0-dev.85`** as the latest recoverable source snapshot. Network protocol is **27**.
...
- dev.85 adds optional EMI/JEI integrations for projector recipes and Crying Obsidian documentation. dev.85 then simplifies the Crying Obsidian viewer UX into one practical setup and ensures the same tutorial page opens from Crying Obsidian, all buds, the cluster and the shard.
...
Then run Windows `build.bat` with Java 21. The static suite verifies the dev.81 source-ID/provider/transform/energy contracts, the dev.82 mod-logo metadata wiring, the dev.85 EMI/JEI integration files, and the previously accepted projector/light regressions.
