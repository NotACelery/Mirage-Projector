...
public final class ImageAssetFormat {
    ...
    public static Kind detect(byte[] bytes) {
        ...
        if (startsWith(bytes, "GIF87a") || startsWith(bytes, "GIF89a")) {
            return Kind.GIF_ANIMATED;
        }
        if (isPng(bytes)) {
            return containsPngChunk(bytes, "acTL") ? Kind.APNG_ANIMATED : Kind.PNG_STATIC;
        }
        if ((bytes[0] & 0xFF) == 0xFF && (bytes[1] & 0xFF) == 0xD8 && (bytes[2] & 0xFF) == 0xFF) {
            return Kind.JPEG_STATIC;
        }
        if (bytes.length >= 2 && bytes[0] == 'B' && bytes[1] == 'M') {
            return Kind.BMP_STATIC;
        }
        if (isWebP(bytes)) {
            return isAnimatedWebP(bytes) ? Kind.WEBP_ANIMATED : Kind.WEBP_STATIC;
        }
        return Kind.UNKNOWN;
    }
    ...
    private static boolean isAnimatedWebP(byte[] bytes) {
        ...
        if (bytes.length >= 21 && ascii(bytes, 12, 4).equals("VP8X") && (bytes[20] & 0x02) != 0) {
            return true;
        }
        int offset = 12;
