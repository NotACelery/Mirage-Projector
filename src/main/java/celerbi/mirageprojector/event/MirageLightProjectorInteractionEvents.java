...
/** Normal RMB cycles light mode; Shift+RMB opens the battery/configuration GUI. */
    ...
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (!event.getLevel().isClientSide && event.getEntity() instanceof ServerPlayer player) {
            if (player.isShiftKeyDown()) {
                ...
                var mode = projector.cycleMode();
                player.displayClientMessage(Component.translatable(mode.hudTranslationKey()), true);
            }
        }
    }
}
