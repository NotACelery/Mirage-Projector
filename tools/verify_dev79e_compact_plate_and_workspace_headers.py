...
# dev.79e: Compact emitters are exterior-facing plates only. Their former top faces
# produced the horizontal striped artifact visible from above, even after volume overlap was removed.
emitters = model["children"]["emitter"].get("elements", [])
need(len(emitters) == 2, "Compact Alt must keep exactly two emitter plates")
if len(emitters) == 2:
    need(set(emitters[0].get("faces", {}).keys()) == {"north"}, "front Compact emitter must render only its outward north face")
    need(set(emitters[1].get("faces", {}).keys()) == {"south"}, "rear Compact emitter must render only its outward south face")
...
for name, text in [("Item", item_screen), ("Banner", banner_screen), ("Entity", entity_screen), ("Image", image_screen)]:
    need("topPos + 30" in text, f"{name} workspace mode/back row is not below the title")
need("private static final int H = 298;" in item_screen, "Item workspace height was not expanded for the second header row")
need("SNAPSHOT_Y = 96" in item_menu and "PLAYER_INV_Y = 202" in item_menu, "Item workspace slots were not shifted with the content")
need("private static final int H = 328;" in banner_screen, "Banner workspace height was not expanded for the second header row")
need("FACE_Y = 102" in banner_menu and "PLAYER_INV_Y = 222" in banner_menu, "Banner workspace slots were not shifted with the content")
need("PANEL_HEIGHT = 466" in entity_screen, "Entity workspace height was not expanded for the second header row")
need("CARD_Y = 87" in entity_menu and "HUMANOID_FIRST_Y = 154" in entity_menu and "PLAYER_INV_Y = 378" in entity_menu, "Entity workspace slots were not shifted with the content")
need("imageHeight = 438" in image_screen and "imageHeight = 314" in image_screen, "Image workspace heights were not expanded for the second header row")
need("244, 62, 0xFF9FBED1" in image_screen, "Image multi-source summary still overlaps the second-row header controls")
...
need('mod_version=0.1.0-dev.79e' in props, 'version is not dev.79e')
need('NETWORK_PROTOCOL = "26"' in main, 'network protocol is not 26')

if errors:
    print('dev.79e projection-state / Compact model verification FAILED')
    for e in errors:
        print(' -', e)
    raise SystemExit(1)
print('dev.79e projection-state / Compact model verification PASS')
