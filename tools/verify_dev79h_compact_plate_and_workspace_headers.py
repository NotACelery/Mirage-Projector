...
# dev.79h: repeated QA showed the decorative Compact Alt emitters were still the likely
...
need(len(emitters) == 0, "Compact Alt emitter visuals must be removed entirely in dev.79h")
front_strip = None
...
need('mod_version=0.1.0-dev.79h' in props, 'version is not dev.79h')
need('NETWORK_PROTOCOL = "26"' in main, 'network protocol is not 26')

if errors:
    print('dev.79h projection-state / Compact model verification FAILED')
    for e in errors:
        print(' -', e)
    raise SystemExit(1)
print('dev.79h projection-state / Compact model verification PASS')
