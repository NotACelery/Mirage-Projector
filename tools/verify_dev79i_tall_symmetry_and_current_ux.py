...
# dev.79i: repeated QA showed the decorative Compact Alt emitters were still the likely
...
need(len(emitters) == 0, "Compact Alt emitter visuals must be removed entirely in dev.79i")
front_strip = None
for element in model["children"]["frame"].get("elements", []):
    if element.get("from") == [5, 2, 3] and element.get("to") == [11, 3, 5]:
        front_strip = element
        break
need(front_strip is not None, "Compact Alt front inner frame strip missing")
if front_strip is not None:
    need("up" not in front_strip.get("faces", {}), "Compact Alt front inner frame strip still exposes an upper face")
upper_base = None
for element in model["children"]["base"].get("elements", []):
    if element.get("from") == [5, 2, 5] and element.get("to") == [11, 3, 11]:
        upper_base = element
        break
need(upper_base is not None, "Compact Alt upper base slab was not shrunk to the non-overlapping 5..11 footprint")
...
# dev.79i: Tall Alt must have symmetric front/rear low translucent boxes.
tall_model = json.loads((ROOT / "src/main/resources/assets/mirage_projector/models/block/tall_mirage_projector_alt.json").read_text())
tall_emitters = tall_model.get("children", {}).get("emitter", {}).get("elements", [])
tall_boxes = {(tuple(e.get("from", [])), tuple(e.get("to", []))) for e in tall_emitters}
need(((6, 2, 2), (10, 4, 4)) in tall_boxes, "Tall Alt front low box is missing")
need(((6, 2, 12), (10, 4, 14)) in tall_boxes, "Tall Alt rear low box is missing")
need(len(tall_emitters) == 2, "Tall Alt must have exactly two low boxes")
...
need('mod_version=0.1.0-dev.79i' in props, 'version is not dev.79i')
need('NETWORK_PROTOCOL = "26"' in main, 'network protocol is not 26')

if errors:
    print('dev.79i Tall symmetry / projection-state verification FAILED')
    for e in errors:
        print(' -', e)
    raise SystemExit(1)
print('dev.79i Tall symmetry / projection-state verification PASS')
