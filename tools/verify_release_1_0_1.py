...
need('mod_version=1.0.1' in props, 'gradle.properties is not 1.0.1')
...
need(len(block_ids) == 18, f'expected 18 registered blocks, got {len(block_ids)}')
need(len(item_ids) == 15, f'expected 15 registered items, got {len(item_ids)}')
...
# 1.0.1 advanced placement foundation.
need('SERIALIZATION_VERSION = 3' in settings, 'ProjectionSettings serialization was not advanced to v3')
for field in ('horizontalOffsetPixels', 'verticalOffsetPixels', 'distanceOffsetPixels'):
    need(field in settings, f'placement field missing: {field}')
need('withPlacement' in settings and 'tiltDegrees()' in settings, 'placement/tilt settings helpers missing')
...
for key in ('offset_horizontal', 'offset_vertical', 'offset_distance', 'tilt', 'reset_position', 'reset_tilt'):
    need(f'gui.mirage_projector.{key}' in screen, f'placement UI control missing: {key}')
renderer = read('src/main/java/celerbi/mirageprojector/client/MirageProjectorRenderer.java')
need(renderer.count('applyProjectionPlacement(') >= 7, 'not all projection render paths use shared placement transform')
need('new Quaternionf(orientation.x(), orientation.y(), orientation.z(), orientation.w())' in renderer, 'projection tilt quaternion is not applied by renderer')
clearance = read('src/main/java/celerbi/mirageprojector/client/ProjectionClearance.java')
need('horizontalOffsetPixels()' in clearance and 'distanceOffsetPixels()' in clearance and 'verticalOffsetPixels()' in clearance, 'clearance does not follow placement offsets')
need('tiltDegrees()' in clearance, 'clearance does not conservatively account for tilt')
...
if errors:
    print('Mirage Projector 1.0.1 release audit FAILED')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)

print(f'Mirage Projector 1.0.1 release audit PASS ({len(json_files)} JSON, {len(block_ids)} blocks, {len(item_ids)} items, {len(langs.get("en_us", {}))} lang keys)')
for note in notes:
    print('NOTE', note)
