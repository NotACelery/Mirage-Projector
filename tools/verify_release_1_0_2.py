...
need('mod_version=1.0.2' in props, 'gradle.properties is not 1.0.2')
...
# 1.0.2 Prism-aware placement foundation.
...
if errors:
    print('Mirage Projector 1.0.2 release audit FAILED')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)

print(f'Mirage Projector 1.0.2 release audit PASS ({len(json_files)} JSON, {len(block_ids)} blocks, {len(item_ids)} items, {len(langs.get("en_us", {}))} lang keys)')
for note in notes:
    print('NOTE', note)
