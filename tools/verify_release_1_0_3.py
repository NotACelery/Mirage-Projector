...
need('mod_version=1.0.3' in props, 'gradle.properties is not 1.0.3')
...
# 1.0.3 Prism-aware placement foundation.
...
need('withPlacement(int verticalPixels, int prismDistancePixels, float tiltDegrees)' in settings, 'corrected placement helper missing')
...
need('prismSpacingAvailable()' in screen and 'PrismProjectionSpacing.minimumDistancePixels(buildSettings())' in screen, 'Prism-only dynamic Distance UI missing')
...
if errors:
    print('Mirage Projector 1.0.3 release audit FAILED')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)

print(f'Mirage Projector 1.0.3 release audit PASS ({len(json_files)} JSON, {len(block_ids)} blocks, {len(item_ids)} items, {len(langs.get("en_us", {}))} lang keys)')
for note in notes:
    print('NOTE', note)
