...
need(any(v in read('gradle.properties') for v in ('mod_version=1.0.31', 'mod_version=1.0.32')), 'release line is not compatible with 1.0.31')
...
need(any(v in read('README.md') for v in ('**1.0.31** is the current implementation snapshot', '**1.0.32** is the current implementation snapshot')), 'README snapshot is not a compatible 1.0.31+ line')
need(any(v in read('docs/ROADMAP.md') for v in ('Current implementation snapshot: **1.0.31**', 'Current implementation snapshot: **1.0.32**')), 'roadmap is not on a compatible 1.0.31+ line')
need(any(v in read('docs/ROADMAP.md') for v in ('Network protocol: **42**','Network protocol: **43**')), 'roadmap protocol is stale')
need(any(v in read('docs/DEVELOPMENT.md') for v in ('Current maintenance baseline: **1.0.31**', 'Current maintenance baseline: **1.0.32**')), 'development baseline is not a compatible 1.0.31+ line')
need(any(v in read('docs/DEVELOPMENT.md') for v in ('Network protocol: **42**','Network protocol: **43**')), 'development protocol is stale')
need(any(v in read('docs/DOCUMENTATION-AUTHORITY.md') for v in ('Documentation Authority — Mirage Projector 1.0.31', 'Documentation Authority — Mirage Projector 1.0.32')), 'documentation authority is not a compatible 1.0.31+ line')
need(any(v in read('docs/CURRENT-IMPLEMENTATION.md') for v in ('Version: **1.0.31**', 'Version: **1.0.32**')), 'current implementation version is not a compatible 1.0.31+ line')
