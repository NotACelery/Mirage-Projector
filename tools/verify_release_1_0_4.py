...
need('mod_version=1.0.4' in props, 'gradle.properties is not 1.0.4')
...
# 1.0.4 fixed-tab placement foundation.
...
need('COLLISION_MARGIN_PIXELS = 0' in spacing, 'Prism +0 baseline is not edge-tight')
...
need('enum SettingsTab' in screen and 'SettingsTab.GEOMETRY' in screen and 'SettingsTab.PLACEMENT' in screen and 'SettingsTab.APPEARANCE' in screen, 'fixed settings tabs missing')
...
print(f'Mirage Projector 1.0.4 release audit PASS ({len(json_files)} JSON, {len(block_ids)} blocks, {len(item_ids)} items, {len(langs.get("en_us", {}))} lang keys)')
for note in notes:
    print('NOTE', note)
