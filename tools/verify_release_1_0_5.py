...
need('mod_version=1.0.5' in props, 'gradle.properties is not 1.0.5')
...
# 1.0.5 fixed-tab placement foundation.
...
print(f'Mirage Projector 1.0.5 release audit PASS ({len(json_files)} JSON, {len(block_ids)} blocks, {len(item_ids)} items, {len(langs.get("en_us", {}))} lang keys)')
for note in notes:
    print('NOTE', note)
