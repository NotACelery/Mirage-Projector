...
need('mod_version=1.0.7' in props, 'gradle.properties is not 1.0.7')
...
need(len(item_ids) == 16, f'expected 16 registered items, got {len(item_ids)}')
...
if errors:
    print('Mirage Projector 1.0.7 release audit FAILED')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)

print(f'Mirage Projector 1.0.7 release audit PASS ({len(json_files)} JSON, {len(block_ids)} blocks, {len(item_ids)} items, {len(langs.get("en_us", {}))} lang keys)')
for note in notes:
    print('NOTE', note)
